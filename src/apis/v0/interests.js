...
module.exports.selectAllInterests = function (request, reply) {
  ...
  var query = ['SELECT interest.*, interest_parent.name AS interest_parent_name, interest_parent.display_name AS interest_parent_display_name',
   'FROM interest',
...
module.exports.selectInterest = function (request, reply) {
  var query = 'SELECT * FROM interest WHERE id = ' + request.params.id;
